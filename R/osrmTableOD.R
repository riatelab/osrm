...
osrmTableOD <- function(dfo, ido, xo, yo, dfd, idd, xd, yd, limit = 100){
  ...
  if((nd <= k)){
    ...
    ## 3
    if (no %% k != 0 ){
      pts <- rbind(dfo[(max(seqi)+k) : no,c(ido, xo, yo)], dfd[,c(idd, xd, yd)])
      x <- osrmTable(df = df, id = "id", x = "x", y = "y" )  
      mati[rownames(mati) %in% rownames(x),colnames(mati) %in% colnames(x)] <- 
        x[rownames(x) %in% rownames(mati),colnames(x) %in% colnames(mati)] 
    }
  ...
  if((nd > k) & (no > k)){
    ...
    for (i in seqi){
      for (j in seqj){
        df <- rbind(dfo[i:(i + (k - 1)),c(ido, xo, yo)], dfd[j:(j + (k-1)),c(idd, xd, yd)])
        x <- osrmTable(df = df, id = "id", x = "x", y = "y" )  
        mati[rownames(mati) %in% rownames(x),colnames(mati) %in% colnames(x)] <- 
          x[rownames(x) %in% rownames(mati),colnames(x) %in% colnames(mati)] 
      }
    ...
    if (nd %% k != 0){
      ...
      for (i in seqi){
        df <- rbind(dfo[i:(i + (k - 1)),c(ido, xo, yo)],
                    dfd[(max(seqj) + k):(max(seqj) + k + (nd %% k) - 1),
                        c(idd, xd, yd)])
        x <- osrmTable(df = df, id = "id", x = "x", y = "y" )  
        mati[rownames(mati) %in% rownames(x),colnames(mati) %in% colnames(x)] <- 
          x[rownames(x) %in% rownames(mati),colnames(x) %in% colnames(mati)] 
    ...
    if (no %% k != 0 ){
      for (j in seqj){
        df <- rbind(dfo[(max(seqi) + k):(max(seqi) + k + (no %% k) - 1),
                        c(ido, xo, yo)],
                    dfd[j:(j + (k-1)),
                        c(idd, xd, yd)])
        x <- osrmTable(df = df, id = "id", x = "x", y = "y" )  
